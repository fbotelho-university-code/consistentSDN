...
\message{ !name(tex/relatedwork.tex) !offset(-152) }
...
\item Interestingly ~\gls{rcp} decouples the control plane from the data plane to solve scalability problem of the \gls{bgp} protocol that required a full meshed network (\bigO{$N^2$} for $N$ routers)   to work correctly. 
...
\message{ !name(../PEI.tex) !offset(-1262) }
